...
class draw_scene:
  def __init__(self,style=1):
    ...
    if len(sys.argv) < 2:
      self.model1.load_stl('cad_models/joystick.stl')
    else:
  ...
  def resize(self, width, height):
    ...
    gluPerspective(120,1.0*width/height, 1,100)
    gluLookAt(self.x, self.y, -20.0, -10, 20, 10, -20, 10.0, 10.0) # xxxxxxxxxxxxxxxxxxxxxx
  ...
  def draw(self):    
    ...
    glLoadIdentity()
    gluLookAt(self.x, self.y, 0.0, 0, 0, 0, 0, 0.0, 100.0) # xxxxxxxxxxxxxxxxxxxxxx

    glTranslatef(-5.0,-40.0, -90.0)
    self.model1.draw()
